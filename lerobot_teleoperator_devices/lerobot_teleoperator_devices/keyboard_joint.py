...
class KeyboardJointTeleop(KeyboardTeleop):
    ...
    def __init__(self, config: KeyboardJointTeleopConfig):
        ...
        self._initialized = False  # Track if we've initialized from robot state
    ...
    def set_initial_state(self, observation: dict[str, Any]) -> None:
        """Initialize teleoperator actions from robot's current state.

        Args:
            observation: Robot observation containing current joint positions
        """
        for key in self.curr_joint_actions.keys():
            if key in observation:
                self.curr_joint_actions[key] = observation[key]
...
class KeyboardJointPandaTeleop(KeyboardJointTeleop):
    """
    Teleop class for Franka Panda robot using keyboard inputs for joint control.
    Supports 7-DOF arm control with gripper.
    """

    config_class = KeyboardJointPandaTeleopConfig
    name = "keyboard_joint_panda"

    def __init__(self, config: KeyboardJointPandaTeleopConfig):
        super().__init__(config)
        self.config = config
