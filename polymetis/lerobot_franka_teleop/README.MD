...
### 1.5 Quick SpaceMouse Control Test

Before full data collection, you can run a minimal SpaceMouse test:

```bash
# 1) Teleop-only: verify SpaceMouse axes/buttons (no robot motion)
franka-test-spacemouse --teleop-only --hz 10

# 2) Robot streaming mode: send SpaceMouse deltas to Franka
franka-test-spacemouse --hz 20
```

Notes:
- The script reads `scripts/config/record_cfg.yaml` for `record.robot` and `record.teleop.spacemouse_config`.
